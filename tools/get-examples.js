 ...
 *     "page": 1,
 *     "results": [
 *       { "url": "https://...", "matchCount": 3, "html": "<article>...</article>" }
 *     ]
 *   }
 *
...
const allPages = entry.mode === 'list'
  ? entry.pages.map(i => index.pageIndex[i])
  : index.pageIndex.filter((_, i) => !new Set(entry.pages).has(i));
...
allPages.sort((a, b) => djb2(a) - djb2(b));
...
for (const url of candidates) {
  ...
  if (skeletonMap.has(skel)) {
    ...
    results.push({ url, ...extracted, duplicates: 0, textVaries: false, _text: extractText(extracted.html) });
  }
}
